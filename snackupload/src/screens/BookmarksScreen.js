...
const BookmarksScreen = () => {
  ...
  useEffect(() => {
    const loadAyaStates = async () => {
      try {
        const storedAyaStates = await StorageService.getAyaStates();
        setAyaStates(storedAyaStates);
      } catch (error) {
        console.error('Error loading ayaStates from StorageService:', error);
      }
    };
  ...
  // Save ayaStates to StorageService whenever it changes
  useEffect(() => {
    const saveAyaStates = async () => {
      try {
        await StorageService.setAyaStates(ayaStates);
      } catch (error) {
        console.error('Error saving ayaStates to StorageService:', error);
      }
    };
